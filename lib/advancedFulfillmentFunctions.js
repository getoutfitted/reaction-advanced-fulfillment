...
// AdvancedFulfillment.date.enoughBizDaysForTransit = function (date, arrivalDate, transitTime) {
//   check(date, Date);
//   check(arrivalDate, Date);
//   check(transitTime, Number);
//   let businessDaysForTransit = 0;
//   let days;
//   while (businessDaysForTransit <= transitTime) {
//     days = AdvancedFulfillment.date.calculateBizDays(date, arrivalDate);
//     date = moment(date).subtract(days.days, 'days').toDate();
//     businessDaysForTransit = days.bizDays;
//   }
//   return moment(date);
// }
...
AdvancedFulfillment.date.determineShipmentDate = function (arrivalDate, transitTime) {
  ...
  // shipmentDate = AdvancedFulfillment.date.enoughBizDaysForTransit(shipmentDate.toDate(), arrivalDate, transitTime);
  shipmentDate = AdvancedFulfillment.date.ifWeekendSetPreviousBizDay(shipmentDate.toDate());
  if (AdvancedFulfillment.date.containsWeekends(shipmentDate, arrivalDate)) {

  }
  // shipmentDate = AdvancedFulfillment.date.addTransitTimeForWeekends(shipmentDate.toDate(), arrivalDate, transitTime);
